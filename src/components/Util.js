...
function formatDisplayDate(rawDate) {
  ...
  return `${date.getDate()} ${date.toLocaleString('en-us', { month: 'long' })} ${currentYear !== repositoryYear ? repositoryYear : ''}`;    
}
...
function replaceRandomCharInWord(mainWord) {
  ...
  const randomIndex = Math.floor(Math.random() * mainWord.length);

  const newWord = mainWord.substring(0, randomIndex)
    + randomCharter
    + mainWord.substring(randomIndex + 1);
